...
function login()
{
	...
	$.post('login.php?login', { user_email: user_email, user_password: user_password, user_remember: user_remember }, function(data)
	{
		...
		else
		{
			if(data == '')
			{
				$('#login_message_p').html('<span class="error_span">Email o password incorrecto.</span>');
...
function toggle_reservation_time(id, week, day, time, from)
{
	...
	else
	{
		...
		{
			if(user_name == '_Espere...')
			{
				notify('Un click es suficiente', 4);
			}
			else if(user_name == session_user_name || session_user_is_admin == '1')
			{
				if(user_name != session_user_name && session_user_is_admin == '1')
				{
					var delete_confirm = confirm('Esta no es su reservación, pero como usted es el administrador puede remover la reservación de otros usuarios.  Está seguro de hacer esto?');
				}
				else
				{
			...
			{
				notify('No puede eliminar la reservación de otro usuario', 2);
			}
...
function read_reservation_details(id, week, day, time)
{
	if(typeof id != 'undefined' && $(id).html() != '' && $(id).html() != 'Wait...')
	{
		if($('#reservation_details_div').is(':hidden'))
		{
			...
			$('#reservation_details_div').html('Obteniendo detalles...');
			...
			$.post('reservation.php?read_reservation_details', { week: week, day: day, time: time }, function(data)
			{
				setTimeout(function()
				{
					if(data == 0)
					{
						$('#reservation_details_div').html('Esta reservación ya no existe. Espere...');
						
						setTimeout(function()
						{
							if($('#reservation_details_div').is(':visible') && $('#reservation_details_div').html() == 'Esta reservación ya no existe. Espere...')
							{
...
function reset_user_password()
{
	if(typeof $(".user_radio:checked").val() !='undefined')
	{
		...
		$('#user_administration_message_p').html('<img src="img/loading.gif" alt="Loading"> Reseteando el password...').slideDown('fast');

		$.post('cp.php?reset_user_password', { user_id: user_id }, function(data)
		{
			if(data == 0)
			{
				$('#user_administration_message_p').html('<span class="error_span">Puede cambiar su password debajo de esta página</span>').slideDown('fast');
			}
			else
			{
...
function change_user_permissions()
{
	if(typeof $(".user_radio:checked").val() !='undefined')
	{
		...
		$.post('cp.php?change_user_permissions', { user_id: user_id }, function(data)
		{
			if(data == 1)
			{
				...
				{
					list_users();
					$('#user_administration_message_p').html('Permisos cambiados correctamente. El usuario debe loguearse nuevamente para obtener los permisos');
				}, 1000);
			}
			else
			{
	...
	{
		$('#user_administration_message_p').html('<span class="error_span">Debe seleccionar un usuario</span>').slideDown('fast');
	}
}
...
function delete_user_data(delete_data)
{
	if(typeof $(".user_radio:checked").val() !='undefined')
	{
		var delete_confirm = confirm('Está seguro?');

		if(delete_confirm)
		{
			var user_id = $(".user_radio:checked").val();

			$('#user_administration_message_p').html('<img src="img/loading.gif" alt="Loading"> Borrando...').slideDown('fast');
