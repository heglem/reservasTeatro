...
// nombre y turno alumno
function datosAlumno(){ 
$user_name=$_SESSION['user_name'];

	$query = mysql_query("SELECT nombre, turno FROM alumnos 
	WHERE numero='".$user_name."'")
	or die('<span class="error_span"><u>MySQL error:</u> ' . htmlspecialchars(mysql_error()) . '</span>');

	if(mysql_num_rows($query) > 0)
	{
		$nom=mysql_fetch_array($query);	
		return $nom;	
	}
}
...
function mapeaAsiento($sector, $asientoC, $fila){
	...
	switch($sector){
		...
		case 6:
			...
			break;
		case 7:
			if($fila=='F06' | $fila=='F07' | $fila=='F08'){
				$sec=array(11,19,17,15,13,11);				
			}else{
...
$query = mysql_query("SELECT reservation_week, reservation_time, reservation_day  
FROM phpmyreservation.phpmyreservation_reservations
where reservation_user_name='".$user_name."'
order by reservation_week")
	or die('<span class="error_span"><u>MySQL error:</u> ' . htmlspecialchars(mysql_error()) . '</span>');
...
$resS='';
	...
	{	
	   ...
	   {		
			$resS=$resS."<tr><td>Sector: ".$r[0]."</td><td> Fila: ".$r[1]."</td><td> Asiento: ".mapeaAsiento($r[0], $r[2],$r[1])."</td></tr>";				
	   }
	}	
return "<table border=1>".$resS."</table>";	
}
...
function validate_alumno($user_name)
{	
...
{	
	if(preg_match('/^[0-9 æøåÆØÅ]{3,6}$/i', $user_name))  // numero entre 3 y 6 digitos
	{
		return(true);
	}
}
...
{
	if(validate_alumno($user_name) != true)
	{
		return('<span class="error_span">No se encontró ningún alumno con el número '.$user_name.'</span>');
	}
	elseif(validate_user_name($user_name) != true)
	{
//		return('<span class="error_span">Name must be <u>letters only</u> and be <u>2 to 12 letters long</u>. If your name is longer, use a short version of your name</span>');
	...
	{
		return('<span class="error_span">El alumno ya fue registrado por otro usuario. Si no fue usted, comuniquese con la administración. Gracias. </span>');
	}
	elseif(user_email_exists($user_email) == true)
	{
		return('<span class="error_span">Ese email ya está registrado. <a href="#forgot_password">Olvidó su passwords?</a></span>');
	}
	else
	{
...
function read_reservation($year, $week, $day, $time)
{
...
function read_reservation_details($year, $week, $day, $time)
{
	$query = mysql_query("SELECT * FROM " . global_mysql_reservations_table . " WHERE reservation_year='$year' AND reservation_week='$week' AND reservation_day='$day' AND reservation_time='$time'")or die('<span class="error_span"><u>MySQL error:</u> ' . htmlspecialchars(mysql_error()) . '</span>');
	$reservation = mysql_fetch_array($query);
	...
	{
//		return('<b>Reservado el:</b> ' . $reservation['reservation_made_time'] . '<br><b>User\'s email:</b> ' . $reservation['reservation_user_email']);
		return('<b>Reservado el:</b> ' . $reservation['reservation_made_time']);
	}
}

function make_reservation($year, $week, $day, $time)
{
	$user_id = $_SESSION['user_id'];
...
//	Control para venta de hasta 5 entradas por alumnno	
	$query = mysql_query("SELECT count(reservation_user_name) FROM " . global_mysql_reservations_table . " WHERE reservation_user_name='$user_name'")or die('<span class="error_span"><u>MySQL error:</u> ' . htmlspecialchars(mysql_error()) . '</span>');	
	$reg = mysql_fetch_array($query);
	
	if($reg[0]>=5){
		return('Ya se reservaron cinco asientos para este alumno');		
	}else{
	...
	if(mysql_num_rows($query) <= 1)
		...
		{
			return('Este asiento ya fue reservado');
		}
...
function delete_reservation($year, $week, $day, $time)
{
	...
	*/
		...
		{
			return('No puede eliminar la reservación de otro usuario');
		}
//	}
}
